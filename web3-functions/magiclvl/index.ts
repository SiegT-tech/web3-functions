...
Web3Function.onRun(async (context: Web3FunctionContext) => {
	...
	const vaultOracle = new Contract(
		userArgs.vaultOracle as string,
		VAULT_ORACLE_ABI,
		provider,
	);
	...
	const stakingLens = new Contract(
		stakingLensAddress,
		STAKING_LENS_ABI,
		provider,
	);
	...
	const oracleImplementation = new Contract(
		await vaultOracle.oracleImplementation(),
		VAULT_ORACLE_ABI,
		provider,
	);
	const vault = new Contract(
		await oracleImplementation.trancheVault(),
		VAULT_ABI,
		provider,
	);
	const timestamp = (await provider.getBlock("latest")).timestamp;
	const lastTimestampStr =
		(await storage.get("lastTimestamp")) ??
		(await harvester.lastExecution()).toString();
	...
	let lvlTokenAmount: BigNumber;
	let wbnbTokenAmount: BigNumber;
	let swapData: string;

	try {
		const pid = getStakingPid(vault.address);
		lvlTokenAmount = (await stakingLens.pendingRewards(pid, vault.address))
			.add(await lvlToken.balanceOf(vault.address))
			.add(await lvlToken.balanceOf(harvester.address));
		console.log(
			"reward amount in LVL",
			(
				(lvlTokenAmount.toString() as unknown as number) / 1e18
			).toLocaleString(),
		);
	} catch {
		return {
			canExec: false,
			message: "pendingRewards call failed",
			callData: [],
		};
	}
	...
	if (wbnbTokenAmount.gt(0)) {
		...
		try {
			...
			const vaultTotalValue = BigNumber.from("10")
				.pow("18")
				.mul(totalSupply)
				.div(peekSpotPrice);
			const vaultTotalValueInFloat =
				(vaultTotalValue.toString() as unknown as number) / 1e18;
			...
			console.log(
				"vault price: $",
				(vaultPrice.toString() as unknown as number) / 1e18,
				", LLP price: $",
				(oneLpPrice.toString() as unknown as number) / 1e18,
			);
			console.log(
				"reward amount in wBNB",
				wbnbTokenAmount.toString(),
				" (",
				(wbnbTokenAmount.toString() as unknown as number) / 1e18,
				")",
			);
			console.log(
				"wBNB price: ",
				`$${((bnbPrice.toString() as unknown as number) / 1e8).toLocaleString()}`,
			);
			console.log(
				"vault total value: ",
				`$${vaultTotalValueInFloat.toLocaleString()}`,
			);
			console.log(
				"reward total value: ",
			...
			if (considerApy) {
				...
				if (apyInBips.gt(maxApyInBipsAsBN)) {
					...
					wbnbTokenAmount = maxApyInBipsAsBN
						...
						.div(bnbPrice);
					console.log(
						`adjusted reward token amount: ${wbnbTokenAmount.toString()}`,
					);
				}
			} else {
			...
			mintLpAmount = wbnbTokenAmount
				.mul(bnbPrice)
				.div(oneLpPrice)
				.mul(BigNumber.from("10000000000"));
		} catch (err) {
